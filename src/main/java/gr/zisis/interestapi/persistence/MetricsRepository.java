...
public interface MetricsRepository extends JpaRepository<Metrics, Integer> {
	...
	@Query(value = "SELECT new gr.zisis.interestapi.controller.response.entity.InterestPerCommitFile(m.sha, m.filePath, m.revisionCount, m.interestEu, m.interestHours) "
			+ "FROM Metrics m "
	...
	@Query(value = "SELECT new gr.zisis.interestapi.controller.response.entity.InterestChangePerCommit(m.sha, m.revisionCount, SUM(m.interestEu) - (SELECT SUM(m2.interestEu) FROM Metrics m2 WHERE m2.pid = (SELECT p.pid FROM Projects p WHERE p.url LIKE ?1) AND m2.revisionCount = m.revisionCount-1), SUM(m.interestHours) - (SELECT SUM(m2.interestHours) FROM Metrics m2 WHERE m2.pid = (SELECT p.pid FROM Projects p WHERE p.url LIKE ?1) AND m2.revisionCount = m.revisionCount-1), (SUM(m.interestEu) - (SELECT SUM(m2.interestEu) FROM Metrics m2 WHERE m2.pid = (SELECT p.pid FROM Projects p WHERE p.url LIKE ?1) AND m2.revisionCount = m.revisionCount-1))/SUM(m.interestEu))"
			+ "FROM Metrics m "
			+ "WHERE m.pid = (SELECT p.pid FROM Projects p WHERE p.url LIKE ?1) AND m.sha = ?2 GROUP BY m.sha, m.revisionCount")
	...
	@Query(value = "SELECT new gr.zisis.interestapi.controller.response.entity.HighInterestFile(m.sha, m.revisionCount, m.filePath, m.interestEu, m.interestHours, m.interestEu/(SELECT SUM(m2.interestEu) FROM Metrics m2 WHERE m2.pid = (SELECT p.pid FROM Projects p WHERE p.url LIKE ?1) AND m2.sha = ?2)) "
			+ "FROM Metrics m "
			+ "WHERE m.pid = (SELECT p.pid FROM Projects p WHERE p.url LIKE ?1) AND sha = ?2 GROUP BY m.sha, m.revisionCount, m.filePath, m.interestEu, m.interestHours ORDER BY m.interestEu DESC")
